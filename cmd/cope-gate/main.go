...
func registerFlags(fs *flag.FlagSet) *options {
	return &options{
		...
		ab:          fs.Bool("ab", false, "rotate refresher windows through the arms and record which arm each turn was written under"),
		abArms:      fs.String("ab-arms", "", "comma-separated arms to rotate through, implying -ab (default inject,hold; positive is the third)"),
		abReport:    fs.String("ab-report", "", "read a turn log and report the arms; - reads the default path"),
		renderArm:   fs.String("render-arm", "", "print the mid-session reminder one arm would inject, and exit"),
